...
class Field {
    ...
    void update_host() {
        ...
        cuda_check_status(cudaMemcpy(host_ptr_, device_ptr_, xdim_*ydim_*sizeof(double), cudaMemcpyDeviceToHost));
    }

    void update_device() {
        // TODO: copy contents of host_ptr_ to device_ptr_ 
        cuda_check_status(cudaMemcpy(device_ptr_, host_ptr_, xdim_*ydim_*sizeof(double), cudaMemcpyHostToDevice));
    }
